...
import com.ar.genesis.sistema.movimiento.service.controller.MovimientoObtenerPorIdController;
...
@SpringBootTest
public class MovimientoObtenerPorIdControllerTest {
    @Mock
    ...
    public void obtenerMovimientoPorId_DevuelveMovimiento() throws MovimientoNoExisteException {
        ...
        Movimiento unMovimiento = Movimiento.instancia(null, unPuntoVenta, TipoMovimiento.instancia(1, "Factura A", "Debe"), 1, unaFicha, unaFicha.getTipoIva(), unaFicha.getCuit(), LocalDate.now(), LocalTime.now(), 200.0, 200.0, 200.0);
        ...
        when(miMovimientoObtenerPorIdInput.obtenerMovimiento(1)).thenReturn(unMovimiento);

        MovimientoObtenerPorIdController movimientoObtenerPorIdController = new MovimientoObtenerPorIdController(miMovimientoObtenerPorIdInput);
        ResponseEntity<?> responseEntity = movimientoObtenerPorIdController.obtenerMovimiento(1);
        MovimientoDTO resultado = (MovimientoDTO) responseEntity.getBody();
        Assertions.assertNotNull(resultado);
    }
    @Test
    public void obtenerMovimientoPorId_DevuelveNull() throws MovimientoNoExisteException {
        when(miMovimientoObtenerPorIdInput.obtenerMovimiento(1)).thenReturn(null);
